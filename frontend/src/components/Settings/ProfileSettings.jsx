...
const ProfileSettings = () => {
  ...
  const handlePhotoChange = async (e) => {
    ...
    try {
      const response = await api.post('/settings/photo', formData, {
        headers: {
      ...
      if (response.success) {
        ...
        const photoURL = `http://localhost:5000${response.photoURL}`;
          ...
          <div className="h-20 w-20 rounded-full bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-2xl font-bold text-white overflow-hidden">
            {userProfile?.profilePicture ? (
              <img
                ...
                onError={(e) => {
                  console.error('Image failed to load:', userProfile.profilePicture);
